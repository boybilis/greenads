...
function printPurchaseOrder(po, previewWin) {
    ...
    const approvedSignature = String(po.approval_status || '').toLowerCase() === 'approved'
        ? '<img class="signature-image" src="dist/img/sir%20bong%20e%20signtransparent.png" alt="Admin approval signature">'
        : '';

    const printable =
        '<!doctype html><html><head><title>' + escapeHtml(po.po_ref_no || 'Purchase Order') + '</title>' +
        '<style>@page{size:A4;margin:14mm}*{box-sizing:border-box}body{margin:0;font-family:Arial,sans-serif;color:#111;font-size:12px}.sheet{width:210mm;min-height:297mm;padding:14mm;margin:0 auto;background:#fff}.header{display:flex;justify-content:space-between;gap:18px;border-bottom:2px solid #111;padding-bottom:12px;margin-bottom:16px}.logo-box{width:115px;height:75px;display:flex;align-items:center;justify-content:center}.logo-box img{max-width:115px;max-height:75px;object-fit:contain}.company{flex:1}h1{margin:0 0 6px;font-size:22px;letter-spacing:0}.meta{text-align:right;line-height:1.6;min-width:165px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:18px;margin-bottom:16px}.section-title{font-weight:bold;margin-bottom:6px;text-transform:uppercase}table{width:100%;border-collapse:collapse;margin-top:10px}th,td{border:1px solid #333;padding:7px;vertical-align:top}th{background:#f2f2f2;text-align:left}.center{text-align:center}.right{text-align:right}.signatures{display:grid;grid-template-columns:1fr 1fr;gap:32px;margin-top:42px}.signature-block{align-items:center;display:flex;flex-direction:column;justify-content:flex-end;min-height:130px}.signature-image{display:block;height:100px;object-fit:contain;width:100px}.signature-line{border-top:1px solid #111;padding-top:6px;text-align:center;width:100%}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}.sheet{width:auto;min-height:auto;padding:0}}</style>' +
        '</head><body onload="window.print();"><div class="sheet">' +
        '<div class="header"><div class="logo-box"><img src="dist/img/greenads_logo.png" alt="Green Ads and Promats Logo"></div><div class="company"><h1>Green Ads and Promats, Inc.</h1><strong>PURCHASE ORDER</strong></div><div class="meta"><div><strong>PO #:</strong> ' + escapeHtml(po.po_ref_no || '') + '</div><div><strong>Date:</strong> ' + escapeHtml(po.po_date || '') + '</div></div></div>' +
        '<div class="grid"><div><div class="section-title">Supplier Details</div><div><strong>VENDOR NAME:</strong> ' + escapeHtml(supplier.supplier_name || '-') + '</div><div><strong>CONTACT PERSON:</strong> ' + escapeHtml(supplier.supplier_owner || '-') + '</div><div>Address: ' + escapeHtml(supplier.address || '-') + '</div><div>Contact No.: ' + escapeHtml(supplier.contact_no || '-') + '</div><div>Email: ' + escapeHtml(supplier.email || '-') + '</div></div><div><div class="section-title">Prepared By</div><div>' + escapeHtml(po.created_by || '') + '</div></div></div>' +
        ...
        '<div class="signatures"><div class="signature-block"><div class="signature-line">Prepared By</div></div><div class="signature-block">' + approvedSignature + '<div class="signature-line">Approved By</div></div></div></div></body></html>';
